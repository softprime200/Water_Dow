...
import net.vrgsoft.videcrop.cropview.util.HandleUtil1;
import net.vrgsoft.videcrop.cropview.util.PaintUtil1;
import net.vrgsoft.videcrop.cropview.window.edge.Edge1;
import net.vrgsoft.videcrop.cropview.window.handle.Handle;

public class CropView1 extends View {
    private static final int SNAP_RADIUS_DP = 6;
    private static final float DEFAULT_SHOW_GUIDELINES_LIMIT = 100.0F;
    private static final float DEFAULT_CORNER_THICKNESS_DP = PaintUtil1.getCornerThickness();
    private static final float DEFAULT_LINE_THICKNESS_DP = PaintUtil1.getLineThickness();
    ...
    public CropView1(Context context, AttributeSet attrs) {
        super(context, attrs);
        mTargetAspectRatio = (float)mAspectRatioX / (float)mAspectRatioY;
        initializedCropWindow = false;
        init(context);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRect(Edge1.LEFT.getCoordinate(), Edge1.TOP.getCoordinate(), Edge1.RIGHT.getCoordinate(), Edge1.BOTTOM.getCoordinate(), mBorderPaint);
        drawCorners(canvas);
    }
    ...
    private void init(Context context) {
        ...
        mHandleRadius = HandleUtil1.getTargetRadius(context);
        mSnapRadius = TypedValue.applyDimension(1, 6.0F, displayMetrics);
        mBorderPaint = PaintUtil1.newBorderPaint(context);
        mGuidelinePaint = PaintUtil1.newGuidelinePaint();
        mBackgroundPaint = PaintUtil1.newBackgroundPaint(context);
        mCornerPaint = PaintUtil1.newCornerPaint(context);
    ...
    private void initCropWindow(Rect bitmapRect) {
        ...
        if (mFixAspectRatio) {
            float halfCropWidth;
            if (AspectRatioUtil1.calculateAspectRatio(bitmapRect) > mTargetAspectRatio) {
                Edge1.TOP.setCoordinate((float)bitmapRect.top);
                Edge1.BOTTOM.setCoordinate((float)bitmapRect.bottom);
                centerX = (float)getWidth() / 2.0F;
                cropWidth = Math.max(40.0F, AspectRatioUtil1.calculateWidth(Edge1.TOP.getCoordinate(), Edge1.BOTTOM.getCoordinate(), mTargetAspectRatio));
                if (cropWidth == 40.0F) {
                    mTargetAspectRatio = 40.0F / (Edge1.BOTTOM.getCoordinate() - Edge1.TOP.getCoordinate());
                }

                halfCropWidth = cropWidth / 2.0F;
                Edge1.LEFT.setCoordinate(centerX - halfCropWidth);
                Edge1.RIGHT.setCoordinate(centerX + halfCropWidth);
            } else {
                Edge1.LEFT.setCoordinate((float)bitmapRect.left);
                Edge1.RIGHT.setCoordinate((float)bitmapRect.right);
                centerX = (float)getHeight() / 2.0F;
                cropWidth = Math.max(40.0F, AspectRatioUtil1.calculateHeight(Edge1.LEFT.getCoordinate(), Edge1.RIGHT.getCoordinate(), mTargetAspectRatio));
                if (cropWidth == 40.0F) {
                    mTargetAspectRatio = (Edge1.RIGHT.getCoordinate() - Edge1.LEFT.getCoordinate()) / 40.0F;
                }

                halfCropWidth = cropWidth / 2.0F;
                Edge1.TOP.setCoordinate(centerX - halfCropWidth);
                Edge1.BOTTOM.setCoordinate(centerX + halfCropWidth);
            }
        } else {
            centerX = 0.1F * (float)bitmapRect.width();
            cropWidth = 0.1F * (float)bitmapRect.height();
            Edge1.LEFT.setCoordinate((float)bitmapRect.left + centerX);
            Edge1.TOP.setCoordinate((float)bitmapRect.top + cropWidth);
            Edge1.RIGHT.setCoordinate((float)bitmapRect.right - centerX);
            Edge1.BOTTOM.setCoordinate((float)bitmapRect.bottom - cropWidth);
        }

    }

    public static boolean showGuidelines() {
        return Math.abs(Edge1.LEFT.getCoordinate() - Edge1.RIGHT.getCoordinate()) >= 100.0F && Math.abs(Edge1.TOP.getCoordinate() - Edge1.BOTTOM.getCoordinate()) >= 100.0F;
    }
    ...
    private void drawRuleOfThirdsGuidelines(Canvas canvas) {
        ...
        float oneThirdCropWidth = Edge1.getWidth() / 3.0F;
        float x1 = left + oneThirdCropWidth;
        canvas.drawLine(x1, top, x1, bottom, mGuidelinePaint);
        float x2 = right - oneThirdCropWidth;
        canvas.drawLine(x2, top, x2, bottom, mGuidelinePaint);
        float oneThirdCropHeight = Edge1.getHeight() / 3.0F;
    ...
    private void onActionDown(float x, float y) {
        ...
        float right = Edge1.RIGHT.getCoordinate();
        float bottom = Edge1.BOTTOM.getCoordinate();
        mPressedHandle = HandleUtil1.getPressedHandle(x, y, left, top, right, bottom, mHandleRadius);
        if (mPressedHandle != null) {
            mTouchOffset = HandleUtil1.getOffset(mPressedHandle, x, y, left, top, right, bottom);
            invalidate();
        }
    }
